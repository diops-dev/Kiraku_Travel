...
// Genere public/sitemap.xml a partir de la table des URL (src/paths.js),
  // dans les trois langues, avec les alternatives hreflang.
  import { writeFileSync } from 'node:fs'
  import { PATHS_L, ROUTE_KEYS, SLUGS_L, SEG_ITIN, SLUGS_MOD_L, SEG_MOD, alternatives } from '../src/paths.js'
  import { LANGS, HTML_LANG } from '../src/langs.js'
  ...
  for (const lang of LANGS) {
    ...
    // Fiches "sejours libertes" (modules CT et extension EX) : absentes du
    // sitemap jusqu'ici alors qu'elles sont routees et indexables (bug releve
    // le 18/09/2026, aucune de ces 17 refs x 3 langues n'apparaissait).
    for (const ref of Object.keys(SLUGS_MOD_L[lang])) {
      entrees.push({ loc: (lang === 'fr' ? '' : '/' + lang) + '/' + SEG_MOD[lang] + '/' + SLUGS_MOD_L[lang][ref], key: 'module', ref, lang })
    }
  }
